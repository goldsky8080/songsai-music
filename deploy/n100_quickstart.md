...
- [nginx.dev-suno-wrapper.conf.example](/d:/music/deploy/nginx.dev-suno-wrapper.conf.example)
...
운영 wrapper 를 직접 수정하면 실제 운영 생성에도 영향이 갈 수 있으므로,
자동 가사 기능을 다시 붙일 때는 테스트 wrapper 를 별도 포트로 하나 더 두는 것이 안전하다.

- 운영 wrapper: `127.0.0.1:3101`
- 테스트 wrapper: `127.0.0.1:3201`
- 운영 공개 주소: `https://suno.songsai.org`
- 테스트 공개 주소: `https://dev-suno.songsai.org`

권장 흐름:

1. `~/services/suno-api` 는 그대로 유지
2. `~/services/suno-api-dev` 같은 별도 디렉터리에서 wrapper 를 하나 더 준비
3. 테스트 wrapper 에만 `gpt_description_prompt` 패치를 적용
4. 필요할 때만 `PORT=3201 npm run dev` 또는 `npm run build && PORT=3201 npm run start` 로 실행
5. 로컬 개발 `.env` 의 `SUNO_API_BASE_URL` 만 `https://dev-suno.songsai.org` 로 바꿔 자동 가사 테스트
6. 검증 완료 후에만 운영 wrapper 로 반영

이 방식이면 운영 앱과 운영 wrapper 는 건드리지 않고,
로컬 개발 테스트만 별도 공개 주소로 안전하게 진행할 수 있다.
...
- AI 자동 가사처럼 wrapper 패치가 필요한 기능은 `dev-suno.songsai.org` 같은 분리된 테스트 wrapper 에서 먼저 검증하는 것이 안전하다.
...
- `suno.songsai.org` 공개 성공
- `dev-suno.songsai.org` 를 추가하면 운영 영향 없이 AI 자동 가사 테스트를 분리해서 진행 가능
